...
if __name__ == "__main__":
    # Only "point" signals are pulled raw (they feed the map); agg_only signals are fetched
    # pre-grouped in 03_rollup (no points, no map).
    keys = sys.argv[1:] or [k for k, s in SIGNALS.items() if s.get("kind") == "point"]
    for k in keys:
        if SIGNALS[k].get("kind") != "point":
            print(f"Skipping '{k}' (agg_only — fetched in 03_rollup)")
            continue
        print(f"Pulling '{k}' …")
        pull(k)
