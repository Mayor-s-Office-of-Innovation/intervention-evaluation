...
def assign(signal_key, polys):
    ...
    detail_cols = [c for c, _ in sig.get("detail", [])]
    seen = set()
    out = []
    ...
    for r in rows:
        ...
        if detail_cols:
            # carried through for the zoom-in marker hover overlay (03_rollup emits them)
            rec["detail"] = {c: r.get(c) for c in detail_cols}
        out.append(rec)
...
if __name__ == "__main__":
    ...
    keys = sys.argv[1:] or [k for k, s in SIGNALS.items() if s.get("kind") == "point"]
    for k in keys:
        if SIGNALS[k].get("kind") != "point":
            print(f"Skipping '{k}' (agg_only)")
            continue
        assign(k, polys)
