...
# "Hot" rate per signal (tuned by eyeballing the live map, D12). Drug-activity calls are
# concentrated (Tenderloin dominant), so a moderate bar focuses on real hotspots.
HOT_RATE = {
    "cfs_drug": 2.0,
}
...
# Presence signal the displacement map is built on (D2 — never the enforcement/arrest signals).
MAP_SIGNALS = {
    "cfs_drug": ["cfs_drug"],
}
MAP_LABEL = {
    "cfs_drug": SIGNALS["cfs_drug"]["label"],
}
